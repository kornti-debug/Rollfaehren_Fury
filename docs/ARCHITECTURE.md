...
- `WeaponDefinition`: implemented — ScriptableObject of weapon stats (fire mode, damage, range, cooldown, aim assist, pellets, spread angle). Assets live in `Assets/Weapons/` (Pistol, Shotgun, Harpoon, Assault Rifle — the rifle still uses the `Flamethrower.asset` file/GUID to preserve scene references).
...
- `SwarmMovement`: implemented — boids flocking auto-attached to each `SimpleEnemy` at runtime; blends separation/alignment/cohesion with a seek toward the ferry (2D for Surface enemies, 3D for Flying). `EnemySpawner` adds procedural ring spawning around the ferry's live position (with a front no-spawn cone), per-wave size variation, ferry-velocity catch-up speed, a rear-enemy speed ramp, and adaptive escalation (clearing a swarm fast doubles the next one). `SimpleHUD` shows a swarm warning banner + the bottom-right weapon panel (RPM) + animated HP/crossing bars; `RoundStartConsole` shows a billboard "Start Crossing" label above the lever.
