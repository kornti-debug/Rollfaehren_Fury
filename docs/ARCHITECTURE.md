...
- `WwiseInitializerRuntime`: implemented — owns one persistent
  `AkInitializer` plus a registered global emitter. Menu title/UI Events use
  that emitter, while gameplay retains spatial scene emitters. Bootstrap play,
  direct scene testing, and returning to Menu therefore share the sound engine
