...
- `WwiseInitializerRuntime`: implemented — guarantees one persistent Wwise
  initializer while Menu and Main keep separate registered emitters and bank
  controllers. Bootstrap play, direct scene testing, and returning to Menu
  therefore share the sound engine without duplicate initializer components.
